...
## 🔒 5. Tính năng Bảo mật & Trải nghiệm Người dùng Mới (Usability & Security Enhancements)

Để nâng cao tính bảo mật và trải nghiệm thực tế khi sử dụng hệ thống KMS, chúng tôi đã tích hợp các cơ chế quản lý phiên làm việc và hồ sơ nâng cao:

### 5.1. Duy trì phiên làm việc qua `localStorage` (Persistent Sessions)
* **Cơ chế:** Thay thế `sessionStorage` bằng `localStorage` để lưu trữ `currentUser`, `keycloakToken` và `isMockLogin`.
* **Trải nghiệm:** Người dùng sẽ không bị đăng xuất khi reload trang, lỡ tay tắt tab hoặc khởi động lại trình duyệt, trừ khi chủ động chọn nút **Đăng xuất (Logout)**.

### 5.2. Bộ đếm thời gian không hoạt động ngầm (Silent Idle Inactivity Tracker)
* **Hoạt động:** Hệ thống tự động theo dõi các sự kiện tương tác của người dùng (`mousemove`, `keydown`, `mousedown`, `scroll`, `click`). Bộ đếm được gia hạn 1 giờ (3600 giây) khi phát hiện thao tác mới.
* **Tự động bảo mật:** Sau 1 giờ không hoạt động, hệ thống chạy ngầm tự động đăng xuất tài khoản, giải phóng token và tự động mở form đăng nhập kèm thông báo chi tiết: *"Phiên làm việc đã hết hạn do không hoạt động trong 1 giờ. Vui lòng đăng nhập lại."*
* **Tối ưu hóa:** Bộ đếm chạy ngầm tần suất 5 giây kiểm tra 1 lần để đảm bảo không gây re-render hay ảnh hưởng tới hiệu năng tải của giao diện.

### 5.3. Mở rộng Hồ sơ Cá nhân (Extended Profile & Bidirectional Email/Phone Sync)
* **Tính năng:** Bổ sung trường nhập liệu và lưu trữ `email` và `phone_number` trong hồ sơ cá nhân.
* **Xử lý chéo:** Tự động kiểm tra trùng lặp email đối với các tài khoản khác trong CSDL cục bộ và tự động gọi API Keycloak `update_keycloak_user_details` để đồng bộ email mới lên máy chủ Keycloak SSO.
