...
*   **`Draggable & Snapping Trigger Button`**
    *   **Nhiệm vụ:** Bong bóng nổi mở Trợ lý AI hỗ trợ kéo thả tự do, phân biệt click và drag dựa trên ngưỡng di chuyển 5px, tự động snap mượt mà (bám dính) vào biên màn hình trái/phải gần nhất (cách biên 24px) khi thả ra và ghi nhớ vị trí cuối cùng qua `localStorage`.
*   **`Graph Node Canvas Tooltip`**
    *   **Nhiệm vụ:** Lớp overlay vẽ trực tiếp trên Canvas đồ thị Obsidian: Tự động chiếu tọa độ node sang tọa độ màn hình thực tế (áp dụng `scale`/`translate`) để vẽ tooltip tối màu chứa `node.label` đầy đủ sắc nét khi hover chuột.

#### D. File `protoc/src/app/App.tsx` (Core Layout and Directory Trees)
*   **`DirectoryTitle` & `FileTreeItem`**
    *   **Nhiệm vụ:** Các component bao bọc tiêu đề thư mục và tệp tin. Tự động lắng nghe sự kiện chuột để hiển thị tooltip dạng `position: fixed` bám theo con trỏ chuột, giúp hiển thị đầy đủ tên bị rút gọn khi hover qua các mục cây thư mục đệ quy mà không lo bị cắt/che khuất bởi sidebar.
