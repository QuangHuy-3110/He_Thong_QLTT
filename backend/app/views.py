...
class ObsidianNotesListAPIView(APIView):
    """
    API View chỉ đọc 100% trực tiếp từ CSDL Supabase Database (LessonPlan attributes).
    """
    def get(self, request):
        from .models import LessonPlan
        ...
        # Lấy 100% Thực thể / Khái niệm từ Supabase DB
        try:
...
class ObsidianNotesByLessonAPIView(APIView):
    """
    API trả về danh sách các note/thực thể RAG liên quan đến một bài giảng cụ thể từ Supabase DB 100%.
    """
    def get(self, request):
        from .models import LessonPlan
